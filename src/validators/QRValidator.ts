...
const validateQRRequest = (
  ...
  const { payload, version, lightcolor, darkcolor, error, mask } = req.body;
  ...
  if (lightcolor !== undefined && !hexColorRegex.test(lightcolor)) {
    return {
      success: false,
      error: "'lightcolor' must be a valid hex color string",
    };
  }
  ...
  if (darkcolor !== undefined && !hexColorRegex.test(darkcolor)) {
    return {
      success: false,
      error: "'darkcolor' must be a valid hex color string",
    };
  }
  ...
  return {
    success: true,
    data: { payload, version, lightcolor, darkcolor, error, mask },
  };
};
